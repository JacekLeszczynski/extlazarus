...
type
  ...
  TMPVCtrlModeMPV = (mmNone, mmCPlayer, mmPseudoGui);
  TMPVCtrlScreenShotFormat = (ssNone,ssJPG,ssPNG);
  TMPVCtrlOnFeedback = procedure(ASender: TObject; AStrings: TStringList) of object;
  TMPVCtrlOnError = procedure(ASender: TObject; AStrings: TStringList) of object;
  TMPVCtrlOnBeforePlay = procedure(ASender: TObject; AFilename: string) of object;
  TMPVCtrlOnPlaying = procedure(ASender: TObject; APosition,ADuration: single) of object;
  TMPVCtrlOnCacheing = procedure(ASender: TObject; APosition,ADuration,ACache: single) of object;
  TMPVCtrlOnGrabImage = procedure(ASender: TObject; AFilename: String) of object;
  TMPVCtrlOnICYRadio = procedure(ASender: TObject; AName,AGenre,AWebsite: string; APublic: boolean; ABitrate, AStreamTitle, AStreamURL: string) of object;
  TMPVCtrlOnCaptureDump = procedure(ASender: TObject; ACapture: boolean) of object;
  TMPVCtrlOnString = procedure(ASender: TObject; AText: String) of object;
  TMPVCtrlProcessPriority = (mpHigh,mpIdle,mpNormal,mpRealTime);

  TCustomMPVControl = class(TComponent)
  private
    FCache: integer;
    FCacheMin: integer;
    ...
    FMPVNoOsc: boolean;
    FOnBeforePlay: TMPVCtrlOnBeforePlay;
    FOnTimerDump: TMPVCtrlOnString;
    FTimerDump: boolean;
    FUniqueKey: string;
    ...
    FNoSound: boolean;
    FOnCapture: TMPVCtrlOnCaptureDump;
    FOnGrabImage: TMPVCtrlOnGrabImage;
    FOnICYRadio: TMPVCtrlOnICYRadio;
    FRadio: boolean;
    FRate: single;
    FStartParam:string;
    FLoop: integer;
    ...
    FStartTime: single;
    FDuration: single;
    FOnError: TMPVCtrlOnError;
    FOnFeedback: TMPVCtrlOnFeedback;
    FOnPlay: TNotifyEvent;
    FOnPlaying: TMPVCtrlOnPlaying;
  ...
  private
    ...
    FOnCacheing: TMPVCtrlOnCacheing;
    FPLAYF: boolean;
    FBostVolume: boolean;
    FModeMPV: TMPVCtrlModeMPV;
    FOnBeforeStop: TNotifyEvent;
    FOnPause: TNotifyEvent;
    FOnReplay: TNotifyEvent;
    FOnSetPosition: TNotifyEvent;
    FPP: TMPVCtrlProcessPriority;
    FscDir: string;
    FSSFormat: TMPVCtrlScreenShotFormat;
  ...
  public
    ...
    property ScreenshotFormat: TMPVCtrlScreenShotFormat read FSSFormat write FSSFormat default ssNone;
    property LastImageFilename: String read FLastImageFilename;

    property ProcessPriority: TMPVCtrlProcessPriority read FPP write FPP default mpNormal;
    property AccelType: string read FAccel write SetAccel;
    property ModeMPV: TMPVCtrlModeMPV read FModeMPV write FModeMPV default mmNone;
    ...
    property OnFeedback: TMPVCtrlOnFeedback read FOnFeedback write FOnFeedback;
    property OnError: TMPVCtrlOnError read FOnError write FOnError;
    property OnPlaying: TMPVCtrlOnPlaying read FOnPlaying write FOnPlaying;
    property OnCacheing: TMPVCtrlOnCacheing read FOnCacheing write FOnCacheing;
    property OnBeforePlay: TMPVCtrlOnBeforePlay read FOnBeforePlay write FOnBeforePlay;
    ...
    property OnGrabImage: TMPVCtrlOnGrabImage read FOnGrabImage write FOnGrabImage;
    property OnICYRadio: TMPVCtrlOnICYRadio read FOnICYRadio write FOnICYRadio;
    property OnCapture: TMPVCtrlOnCaptureDump read FOnCapture write FOnCapture;
    property OnTimerDump: TMPVCtrlOnString read FOnTimerDump write FOnTimerDump;
    property OnSetPosition: TNotifyEvent read FOnSetPosition write FOnSetPosition;
  end;

  TMPVControl = class(TCustomMPVControl)
  published
  ...
  {$ifdef Linux}
  TWSMPVControl = class(TGtk2WSWinControl)
  published
...
procedure Register;
begin
  RegisterComponents('Multimedia',[TMPVControl]);
end;
...
procedure TCustomMPVControl.TimerEvent(Sender: TObject);
var
  a,b,c: single;
begin
...
procedure TCustomMPVControl.PlayerProcessReadData(Sender: TObject);
var
  ErrList: TStringList;
  i,j: integer;
...
function TCustomMPVControl.ExecuteSockProcess(command: string;
...
function TCustomMPVControl.GetDeinterlace: boolean;
begin
  result:=FDeinterlace;
end;

procedure TCustomMPVControl.SetAccel(AValue: string);
begin
  if AValue='' then FAccel:='<auto>' else FAccel:=AValue;
end;

procedure TCustomMPVControl.SetDeinterlace(AValue: boolean);
var
  s: string;
begin
...
procedure TCustomMPVControl.SetscDir(AValue: string);
var
  s: string;
begin
...
procedure TCustomMPVControl.WMPaint(var Message: TLMPaint);
begin
...
procedure TCustomMPVControl.WMSize(var Message: TLMSize);
begin
  if (Message.SizeType and Size_SourceIsInterface)>0 then
    DoOnResize;
end;

procedure TCustomMPVControl.SetStartParam(const AValue: string);
begin
  if FStartParam=AValue then exit;
  FStartParam:=AValue;
end;

procedure TCustomMPVControl.SetFilename(const AValue: string);
...
procedure TCustomMPVControl.SetLoop(const AValue: integer);
begin
...
procedure TCustomMPVControl.SetMPlayerPath(const AValue: string);
begin
  if FMPlayerPath=AValue then exit;
  FMPlayerPath:=AValue;
end;

procedure TCustomMPVControl.SetPaused(const AValue: boolean);
begin
...
procedure TCustomMPVControl.SetRate(AValue: single);
begin
...
procedure TCustomMPVControl.SetVolume(const AValue: integer);
var
  s: string;
begin
...
constructor TCustomMPVControl.Create(TheOwner: TComponent);
var
  vKey: TGuid;
begin
...
destructor TCustomMPVControl.Destroy;
begin
  Stop;
  FreeAndNil(FCanvas);
  FreeAndNil(FTimer);
...
function TCustomMPVControl.GetMpvIpcDevFile: string;
begin
  result:=GetFileMpvSocket;
end;

procedure TCustomMPVControl.SendMPlayerCommand(Cmd: string);
begin
  if Cmd='' then exit;
...
function TCustomMPVControl.Running: boolean;
begin
  Result:=(fPlayerProcess<>nil) and fPlayerProcess.Running;
end;

function TCustomMPVControl.FindMPlayerPath: Boolean;
var
  ExePath: string;
  MPlayerExe: String;
begin
...
function TCustomMPVControl.FindMPVPath: Boolean;
var
  ExePath: string;
  MPlayerExe: String;
begin
...
procedure TCustomMPVControl.GrabImage;
begin
...
procedure TCustomMPVControl.Play(sBaseDirectory: string);
var
  CurWindowID: PtrUInt;
...
begin
  ...
  {$IFDEF DEBUG}
  DebugLn(['TCustomMPVControl.Play ']);
  {$ENDIF}
  {$endif}

  if fPlayerProcess<>nil then
    FreeAndNil(fPlayerProcess);
//    raise Exception.Create('TCustomMPVControl.Play fPlayerProcess still exists');
  ...
  {$IFDEF DEBUG}
  DebugLn(['TCustomMPVControl.Play ', FPlayerProcess.Parameters.DelimitedText]);
  {$ENDIF}
...
procedure TCustomMPVControl.Stop;
var
  s: string;
begin
  if FPlayerProcess=nil then
...
function TCustomMPVControl.Playing: boolean;
begin
  Result := Running and (not Paused);
end;

procedure TCustomMPVControl.Invalidate;
begin
  if csCustomPaint in FControlState then exit;
  inherited Invalidate;
end;

procedure TCustomMPVControl.EraseBackground(DC: HDC);
begin
  if (FCanvas <> nil) then
    with FCanvas do
    begin
...
procedure TCustomMPVControl.Capturing;
begin
  vIsDump:=not vIsDump;
  SendMPlayerCommand('capturing');
end;

procedure TCustomMPVControl.Pause;
var
  s: string;
begin
  if FEngine=meMplayer then
...
procedure TCustomMPVControl.Replay;
var
  s: string;
begin
...
function TCustomMPVControl.GetPositionOnlyRead: single;
begin
  result:=FPosition;
end;

procedure TCustomMPVControl.SetOSDLevel(aLevel: word);
begin
  ExecuteSockProcess('{ "command": ["set_property", "osd-level", '+IntToStr(aLevel)+'] }');
end;

procedure TCustomMPVControl.SetMute(aMute: boolean);
var
  s: string;
begin
...
procedure TCustomMPVControl.SetChannels(aChannels: word);
begin
  if FNosound then exit;
  case aChannels of
    0: SetMute;
...
function TCustomMPVControl.GetAudioSamplerate: integer;
var
  s,s2: string;
  jData: TJSONData;
  jObject: TJSONObject;
...
procedure TCustomMPVControl.SetAudioSamplerate(aSamplerate: integer);
begin
  if FNosound then exit;
  ExecuteSockProcess('{ "command": ["set_property", "audio-samplerate", '+IntToStr(aSamplerate)+'] }');
end;

procedure TCustomMPVControl.SetAudioEQ(s: string);
begin
  if FNosound then exit;
  if s='' then
...
procedure TCustomMPVControl.SetAF(aValue: string);
begin
  if FNosound then exit;
  if aValue='' then
...
procedure TCustomMPVControl.AddAF(aFilter: string; aValue: string);
begin
  if FNosound then exit;
  if aValue='' then
...
function TCustomMPVControl.SingleMpToTime(AValue: single): TTime;
begin
  result:=AValue/SecsPerDay;
end;

function TCustomMPVControl.SingleMpToInteger(AValue: single): integer;
begin
  result:=TimeToInteger(SingleMpToTime(AValue));
end;

function TCustomMPVControl.TimeToSingleMp(AValue: TTime): single;
begin
  result:=SecsPerDay*AValue;
end;

function TCustomMPVControl.IntegerToSingleMp(AValue: integer): single;
begin
  result:=TimeToSingleMp(IntegerToTime(AValue));
end;

procedure TCustomMPVControl.SetPositionEx(aPosition, aMax: integer);
begin
  SetPosition(round(FDuration*aPosition/aMax));
end;

procedure TCustomMPVControl.RecordStart(aFilename: string);
begin
  ExecuteSockProcess('{ "command": ["record-start", "'+aFilename+'"] }');
  //ExecuteSockProcess('{ "command": ["record-file", "'+aFilename+'"] }');
end;

procedure TCustomMPVControl.RecordStop;
begin
  ExecuteSockProcess('{ "command": ["record-stop"] }');
end;

procedure TCustomMPVControl.InitialiseInfo;
begin
  FLastPosition := '';
  FPosition := 0;
  FNetCache := 0;
...
function TCustomMPVControl.GetFileMpvSocket: string;
var
  s: string;
begin
...
function TCustomMPVControl.GetPosition: single;
var
  s,s2: string;
  jData: TJSONData;
  jObject: TJSONObject;
...
function TCustomMPVControl.GetCache: single;
var
  s,s2: string;
  jData: TJSONData;
  jObject: TJSONObject;
...
function TCustomMPVControl.GetDuration: single;
var
  s,s2: string;
  jData: TJSONData;
  jObject: TJSONObject;
...
function TCustomMPVControl.GetRate: single;
begin
  Result := FRate;
...
procedure TCustomMPVControl.SetImagePath(AValue: string);
begin
...
procedure TCustomMPVControl.SetMpvIpcDevFile(AValue: string);
begin
  if FMpvIpcDevFile=AValue then Exit;
  if AValue='' then FMpvIpcDevFile:='<auto>' else FMpvIpcDevFile:=AValue;
end;

procedure TCustomMPVControl.SetPosition(AValue: single);
var
  s: string;
begin
  if Running then
...
class function TWSMPVControl.CreateHandle(const AWinControl: TWinControl;
...
class procedure TWSMPVControl.DestroyHandle(const AWinControl: TWinControl
  );
begin
...
initialization
  {$ifdef Linux}
  RegisterWSComponent(TCustomMPVControl,TWSMPVControl);
  {$endif}
